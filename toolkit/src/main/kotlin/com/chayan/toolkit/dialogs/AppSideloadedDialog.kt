...
import com.chayan.toolkit.compose.alert_dialog.*
import com.chayan.toolkit.compose.components.LinkifyTextComponent
import com.chayan.toolkit.compose.extensions.DEVELOPER_PLAY_STORE_URL
import com.chayan.toolkit.compose.extensions.MyDevices
import com.chayan.toolkit.compose.theme.AppThemeSurface
import com.chayan.toolkit.databinding.DialogTextviewBinding
...
class AppSideloadedDialog(val activity: Activity, val callback: () -> Unit) {
    ...
    init {
        val view = DialogTextviewBinding.inflate(activity.layoutInflater, null, false).apply {
            val text = String.format(activity.getString(R.string.sideloaded_app), DEVELOPER_PLAY_STORE_URL)
    ...
    private fun downloadApp() {
        activity.launchViewIntent(DEVELOPER_PLAY_STORE_URL)
    }
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(onClick = {
                ...
                onDownloadClick(DEVELOPER_PLAY_STORE_URL)
            }) {
        ...
        shape = dialogShape,
        text = {
            val source = stringResource(id = R.string.sideloaded_app, DEVELOPER_PLAY_STORE_URL)
